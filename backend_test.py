...
class BackendTester:
    ...
    async def test_user_profile_endpoints(self):
        ...
        response = await self.make_request("GET", "/profile", token=self.user_token)
        ...
        response = await self.make_request("PUT", "/profile", update_data, token=self.user_token)
        ...
        response = await self.make_request("GET", f"/users/{self.test_user_id}/profile")
        if response and "id" in response:
        ...
        response = await self.make_request("GET", f"/users/{self.test_user_id}/questions")
        ...
        response = await self.make_request("GET", f"/users/{self.test_user_id}/answers")
        ...
        response = await self.make_request("GET", f"/users/{self.test_user_id}/quiz-attempts")
    ...
    async def test_notification_endpoints(self):
        ...
        response = await self.make_request("GET", "/notifications", token=self.user_token)
        ...
        response = await self.make_request("GET", "/notifications/count", token=self.user_token)
        ...
        await self.create_test_quiz_for_notifications()
        
        # Test 4: Mark all notifications as read
        response = await self.make_request("PUT", "/notifications/mark-all-read", token=self.user_token)
    ...
    async def create_test_quiz_for_notifications(self):
        ...
        response = await self.make_request("POST", "/admin/quiz", quiz_data, token=self.admin_token)
        ...
        # Publish the quiz
        response = await self.make_request("POST", f"/admin/quiz/{quiz_id}/publish", token=self.admin_token)
        if not response:
        ...
        response = await self.make_request("POST", f"/quiz/{quiz_id}/attempt", attempt_data, token=self.user_token)
        if response and "id" in response:
            self.log_test("Quiz Attempt for Notifications", True, f"Quiz completed with {response['percentage']}% score")
            
            # Check if notification was created
            await asyncio.sleep(1)  # Give time for notification to be created
            notifications = await self.make_request("GET", "/notifications", token=self.user_token)
            if notifications:
                ...
                if quiz_notifications:
                    ...
                    response = await self.make_request("PUT", f"/notifications/{notification_id}/read", token=self.user_token)
                    if response and "message" in response:
                    ...
                    response = await self.make_request("DELETE", f"/notifications/{notification_id}", token=self.user_token)
                    if response and "message" in response:
        ...
        await self.make_request("DELETE", f"/admin/quiz/{quiz_id}", token=self.admin_token)
    ...
    async def test_notification_triggers(self):
        ...
        await self.test_qa_notifications()
    
    async def test_qa_notifications(self):
        ...
        response = await self.make_request("POST", "/questions", question_data, token=self.user_token)
        if not response or "id" not in response:
        ...
        response = await self.make_request("POST", f"/questions/{question_id}/answers", answer_data, token=self.admin_token)
        if response and "id" in response:
            ...
            response = await self.make_request("PUT", f"/answers/{answer_id}/accept", token=self.user_token)
            if response:
                ...
                admin_notifications = await self.make_request("GET", "/notifications", token=self.admin_token)
        ...
        await self.make_request("DELETE", f"/questions/{question_id}", token=self.user_token)
    ...
    async def test_profile_statistics(self):
        ...
        profile = await self.make_request("GET", "/profile", token=self.user_token)
        if profile:
    ...
    async def run_comprehensive_tests(self):
        ...
        # Run all test suites
        await self.test_user_profile_endpoints()
        await self.test_notification_endpoints()
        await self.test_notification_triggers()
        await self.test_profile_statistics()
