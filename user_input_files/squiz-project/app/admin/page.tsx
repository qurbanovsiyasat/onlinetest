...
export default function AdminPage() {
  ...
  const handleQuizPermissionToggle = async (userId: string, canCreateQuiz: boolean) => {
    try {
      setUsers((prev) => prev.map((user) => (user.id === userId ? { ...user, can_create_quiz: canCreateQuiz } : user)))
      toast({
        title: "Uğurlu",
        description: `Quiz yaratma icazəsi ${canCreateQuiz ? "verildi" : "alındı"}`,
      })
    } catch (error) {
      console.error("Quiz icazəsi dəyişdirmə xətası:", error)
      toast({
        title: "Xəta",
        description: "Quiz icazəsi dəyişdirilə bilmədi",
        variant: "destructive",
      })
    }
  }
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-2 justify-end">
                          <div className="flex items-center gap-3">
                            ...
                            {user.role !== "admin" && (
                              <div className="flex flex-col items-center gap-1">
                                <Switch
                                  checked={user.can_create_quiz === true}
                                  onCheckedChange={(checked) => handleQuizPermissionToggle(user.id, checked)}
                                  size="sm"
                                />
